    ...
    struct thread
      {
        ...
        int64_t sleep_time;     		/* Thread required sleeping time.*/
        ...
        /*Advanced priority data structures.  */
        int nice;				/* Thread nice value. */
        real_t recent_cpu;			/* Thread calculated recent cpu time.*/
    ...
    /* Assigns list_less_func. */
    bool compare_threads_sleep_time(const struct list_elem *a,
                                  const struct list_elem *b,
                                  void *aux);
    ...
    /* :::MLFQS::: Increments Thread recent CPU time,
     on every timer_tick()*/
    void increment_recent_cpu(void);
     
    /* :::MLFQS::: Calculates System Load Average when
     the system tick counter reaches a multiple of a second,
     that is, when timer_ticks () % TIMER_FREQ == 0,
     and not at any other time. */
    void calculate_load_avg(void);
     
    /* :::MLFQS::: Calculates Thread recent CPU time,
     when the system tick counter reaches a multiple of
     a second, that is, when timer_ticks () % TIMER_FREQ == 0,
     and not at any other time. */		
    void calculate_thread_recent_cpu(void);
     
    /* :::MLFQS::: Updates Threads priority.
